...
> **Read [`26-m1-novelty-gate.md`](26-m1-novelty-gate.md) before this section.** The novelty gate retired the broad architectural claim: AgentBound already confines unmodified MCP servers with concrete runtime permissions, Progent already does per-call argument policy, and staging-and-commit for filesystem effects was already done by Alcatraz (2003, process layer) and TxOS (2009, kernel syscall layer). SAFEFLOW does transactional agent execution, but — verified from its full text, [`26`](26-m1-novelty-gate.md) §5.1 — its rollback restores internal agent bookkeeping, never a real external effect a tool actually produced; it does not compete with the narrow candidate below, it sits one layer above it. What survives is a measurement and evaluation contribution plus one narrow candidate mechanism, now precisely bounded by three adjacent systems rather than asserted against none, and the program below should be read with that correction applied.
