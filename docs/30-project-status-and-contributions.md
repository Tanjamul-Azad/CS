...
tested a mechanism for that gap: a specification-ladder contract, enforced
...
5. **Two real vulnerabilities in this project's own mechanism, found by
   adversaries built to know that mechanism's code, not assumed safe
   because it passed every attack written before it existed**: a TOCTOU
   race in the M2 mediator's commit logic (`32`), landing attacker content
   in the trusted store in 1 of 4 trials, fixed and re-verified at 0 of
   20; and a replay/allowance-exhaustion double-commit (`38`) — the
   diff-only mediator cannot tell a replay of an already-used
   authorization from its first legitimate use, so it committed twice,
...
| **M4** — adaptive adversary | **Three of four named attack families tried.** Two found-and-fixed (TOCTOU race `32`, 25%→0%; replay/allowance-exhaustion `38`, via the existing `AllowanceLedger`). One demonstrated as a structural scope boundary, not a bug: unmediated-channel exfiltration (`39`) — a secret sent over a local socket is invisible to the file-only mediator, which commits a perfectly clean honest write in the same invocation. Utility degradation untried |
...
- **Adaptive adversary: three of four named families tried, one not.**
  `32` found and fixed a TOCTOU race (25%→0%); `38` found and fixed a
  replay/allowance-exhaustion double-commit; `39` demonstrated unmediated-
  channel exfiltration directly — a secret sent over a local socket is
  invisible to the file-only mediator, no fix attempted because none
  applies within this mechanism's own layer. Utility degradation, named
  in `25` §6 M4, remains untried. Every attack tested before `32` (M0
...
| Full narrative history | `docs/25` (plan) → `26` (novelty gate) → `27` (candidate spec) → `28` (κ result) → `29` (real-server pilot) → `31` (M2 proper) → `32` (M4 TOCTOU) → `33` (security argument) → `34` (corner cases) → `35` (mitigation strategies) → `36` (our approach, clean spec) → `37` (M2 + real server) → `38` (M4 replay) → `39` (M4 unmediated channel) → this document |
