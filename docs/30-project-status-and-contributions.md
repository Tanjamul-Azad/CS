...
- **Adaptive adversary: all four named families tried, two are
  structural, not bugs.** `32` found and fixed a TOCTOU race (25%→0%);
  `38` found and fixed a replay/allowance-exhaustion double-commit; `39`
  and `40` demonstrated two genuine, unfixable-within-this-mechanism
  limits — unmediated-channel exfiltration, and a fixed timing window
