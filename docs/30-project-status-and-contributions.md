...
5. **A real vulnerability in this project's own mechanism, found by an
   adversary built to know that mechanism's code, not assumed safe
   because it passed every attack written before it existed** (`32`): a
   TOCTOU race in the M2 mediator's commit logic, landing attacker
   content in the trusted store in 1 of 4 trials — then fixed, and the
   fix re-run against the identical attack rather than trusted on
   inspection, dropping to 0 of 20.
...
- **Adaptive adversary: one attack family tried, others not.** `32`
  found and fixed a real TOCTOU race in the M2 mediator (25% win rate,
  fixed to 0%), but that is one of several attack strategies `25` §6 M4
  names — unmediated side channels, allowance exhaustion, utility
  degradation are all still untried. Every attack tested before `32` (M0
...
| Full narrative history | `docs/25` (plan) → `26` (novelty gate) → `27` (candidate spec) → `28` (κ result) → `29` (real-server pilot) → `31` (M2 proper) → `32` (M4 first attack) → `33` (security argument) → `34` (corner cases) → `35` (mitigation strategies) → `36` (our approach, clean spec) → `37` (M2 + real server) → this document |
