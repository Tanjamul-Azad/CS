...
| C11 | The mechanism transfers across heterogeneous unmodified servers. | matched five-condition run on five frozen filesystem-class servers (`artifact/results/matched_filesystem.json`): MCPGate 31/31 prevented, no false block | READY (filesystem) | The filesystem arm establishes transfer across two exact-write and three multi-file tree servers. Network and SQL arms remain to broaden the class beyond the filesystem. |
